...
def self.fetch_results (params,paramsdate)
	...
	rides = Ride.find_by_sql("SELECT * from #{tablename} WHERE DEPART_CITY like '%#{params[:from_city]}%' AND ARRIVE_CITY like '%#{params[:to_city]}%'")
	return rides.to_a

end

def self.fetch_back_results params
	rides = Ride.find_by_sql("SELECT * from #{tablename} WHERE DEPART_CITY like '%#{params[:to_city]}%' AND ARRIVE_CITY like '%#{params[:from_city]}%'")
	return rides
end
